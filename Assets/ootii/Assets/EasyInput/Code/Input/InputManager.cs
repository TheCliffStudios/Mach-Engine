...
#pragma warning restore 0414
        ...
        public static float MovementX
        {
            get
            {
                ...
                float lMovement = (lMin * -1) + lMax;
                if (lMin != 0f || lMax != 0f) { return lMovement; }

                lMovement = GetValue("_MoveHorizontal");
                return lMovement;
            }
        }
        ...
        public static float MovementY
        {
            get
            {
                ...
                float lMovement = lMin + (lMax * -1);
                if (lMin != 0f || lMax != 0f) { return lMovement; }

                lMovement = GetValue("_MoveVertical");
                return lMovement;
            }
        }
        ...
        /// <summary>
        /// Determines if the Xbox controller is active at all.
        /// </summary>
        /// <returns></returns>
        public static bool IsXboxActive()
        {
            if (mState.Controls[EnumInput.GAMEPAD_LEFT_STICK_X].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_LEFT_STICK_Y].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_LEFT_STICK_BUTTON].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_RIGHT_STICK_X].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_RIGHT_STICK_Y].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_RIGHT_STICK_BUTTON].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_LEFT_TRIGGER].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_RIGHT_TRIGGER].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_0_BUTTON].Value != 0f) { return true; } // A
            if (mState.Controls[EnumInput.GAMEPAD_1_BUTTON].Value != 0f) { return true; } // B
            if (mState.Controls[EnumInput.GAMEPAD_2_BUTTON].Value != 0f) { return true; } // X
            if (mState.Controls[EnumInput.GAMEPAD_3_BUTTON].Value != 0f) { return true; } // Y
            if (mState.Controls[EnumInput.GAMEPAD_BACK_BUTTON].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_START_BUTTON].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_LEFT_BUMPER].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_RIGHT_BUMPER].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_DPAD_X].Value != 0f) { return true; }
            if (mState.Controls[EnumInput.GAMEPAD_DPAD_Y].Value != 0f) { return true; }
            return false;
        }
        ...
        public static void SendEvents()
        {
            ...
            //
            // Listeners would look like this:
            // MessageDispatcher.AddListener(EnumInput.MOUSE_LEFT_BUTTON.ToString(), EnumInputMessageType.INPUT_JUST_PRESSED, OnMouseClick);
            // MessageDispatcher.AddListener(EnumInput.MOUSE_LEFT_BUTTON.ToString(), EnumInputMessageType.INPUT_JUST_RELEASED, OnMouseRelease);
            //
            // Listeners for aliases would look like this:
            // MessageDispatcher.AddListener("Jump", EnumInputMessageType.INPUT_JUST_PRESSED, OnJump);
            //
            // ******************************************************************************
            ...
            //    if (IsJustPressed(i))
            //    {
            //        Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_PRESSED, i.ToString(), mState.Controls[i], 0f);

            //        // NOTE: Alterante approach
            //        // This approach sends a message without caring about the actual key/control. It's useful for one function
            //        // that captures all input. In this case, you would add a listener like this:
            //        // MessageDispatcher.AddListener(EnumInputMessageType.INPUT_JUST_PRESSED, OnInputPressed);

            //        //Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_PRESSED, mState.Controls[i], 0f);
            //    }

            //    if (IsJustReleased(i))
            //    {
            //        Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_RELEASED, i.ToString(), mState.Controls[i], 0f);

            //        // NOTE: Alterante approach
            //        // This approach sends a message without caring about the actual key/control. It's useful for one function
            //        // that captures all input. In this case, you would add a listener like this:
            //        // MessageDispatcher.AddListener(EnumInputMessageType.INPUT_JUST_RELEASED, OnInputReleased);

            //        //Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_RELEASED, mState.Controls[i], 0f);
            //    }
            //}

            //// Cycle through all the aliases
            //if (Aliases.Count > 0)
            //{
            //    string[] lAliases = new string[Aliases.Count];
            //    Aliases.Keys.CopyTo(lAliases, 0);

            //    for (int i = 0; i < lAliases.Length; i++)
            //    {
            //        if (IsJustPressed(lAliases[i]))
            //        {
            //            Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_PRESSED, lAliases[i], null, 0f);
            //        }

            //        if (IsJustReleased(lAliases[i]))
            //        {
            //            Messages.MessageDispatcher.SendMessage(null, EnumInputMessageType.INPUT_JUST_RELEASED, lAliases[i], null, 0f);
            //        }
            //    }
            //}
        }
