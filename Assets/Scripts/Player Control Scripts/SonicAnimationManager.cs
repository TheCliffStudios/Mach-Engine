...
public class SonicAnimationManager : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (_PC._Ball)
        {
            ...
            if ((Time.time*10)%5 != 0)
            {
                _BallBody.SetActive( !_BallBody.activeSelf);
            }
        }
        else
        {
        ...
        {
            _BallBody.transform.RotateAround(_BallBody.transform.position, _BallBody.transform.right, 2000 * Time.deltaTime);
        }
                ...
                if (_PC._PlayerState == PlayerControler.PlayerState.Normal)
                {
                    ...
                    else
                    {
                        if (_PC._Grounded)
                        {
                            ...
                            else if (_PC.Velocity.magnitude < (_PC.MaximumSpeed * (0.2f)))
                            {
                                SetSpeed(_PC.Velocity.magnitude / 4);
                                Play("Jog_Loop");
                            }
                            else if (_PC.Velocity.magnitude < (_PC.MaximumSpeed * (0.4f)))
                            {
                                SetSpeed(_PC.Velocity.magnitude / 4);
                                Play("Run_Loop");
                            }
