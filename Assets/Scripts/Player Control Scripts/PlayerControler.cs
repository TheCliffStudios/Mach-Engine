...
public class PlayerControler : MonoBehaviour {
    ...
    public Control2DOverrideZone _2DZone;
    ...
    private void Update()
    {
        ...
        if (_ValidTarget && !_Grounded && _PlayerState == PlayerState.Normal)
        {
    ...
    void TakeInput()
    {
        ...
        if (_2DZone == null) //3D Input
        {
            if (InputManager.IsJustPressed("Jump"))
            {
                if (_PlayerState == PlayerState.SpinDash)
                {
                    SpindashPower += 5;
                }
                ...
                if (((_PlayerState == PlayerState.Normal || _PlayerState == PlayerState.WallSlide) && _Grounded) || _PlayerState == PlayerState.RailGrinding)
                {
                    Velocity += _GroundNormal * 10;
            ...
            if (InputManager.IsJustPressed("GroundPound"))
            {
                if (!_Grounded)
                {
                ...
                else if (InputManager.IsJustPressed("GroundPound") && _Grounded && !_Ball)
                {
            ...
            if (InputManager.IsJustPressed("Ball"))
            {
            ...
            else if (InputManager.IsJustReleased("Ball"))
            {
                if (_PlayerState == PlayerState.Normal)
                {
                    _Ball = false;
            ...
            if (InputManager.IsJustPressed("LeftStep"))
            {
                if (_PlayerState == PlayerState.Normal)
                {
                    _PlayerState = PlayerState.LeftStep;
                    StepTimer = 0;
                }
            }
            if (InputManager.IsJustPressed("RightStep"))
            {
                if (_PlayerState == PlayerState.Normal)
                {
                    _PlayerState = PlayerState.RightStep;
                    StepTimer = 0;
                }
            }
    ...
    void GeneralPhysics()
    {
        ...
        else if (_PlayerState == PlayerState.RightStep || _PlayerState == PlayerState.LeftStep)
        {
            SideStepPhysics();
        }
    }
    ...
    void NormalPhysics()
    {
        ...
        if (Physics.Raycast(transform.position + -RayDirection * 0.5f, RayDirection, out _Hit, RayRange, _Ground))
        {
            ...
            if (AirVelocity.magnitude < 1 && !Jumping) AirVelocity = Vector3.zero;
    ...
    void RunInput()
    {
        ...
        else
        {
            ...
            if (_Grounded)
            {
                Vector3 Forward = Vector3.Cross(_GroundNormal, -Vector3.Cross(_CameraTransformDuplicate.Up, _CameraTransformDuplicate.Forward)) * _RawInput.z;
                Vector3 Right = Vector3.Cross(_GroundNormal, _CameraTransformDuplicate.Forward) * _RawInput.x;
                ...
                if (GV.magnitude < ControlSpeed)
                {
                    ...
                    GV = Turn(GV, Input, TurningSpeed, Mag);
                }
                else
                {
                    float Mag = GV.magnitude;
            ...
            else
            {
                ...
                if (GV.magnitude < ControlSpeed)
                {
                    ...
                    Mag = Mathf.Clamp(GV.magnitude, Mag, MaximumSpeed);
                    GV = Turn(GV, Input, AirTurningSpeed, Mag);
                }
                else
                {
                    float Mag = GV.magnitude;

                    GV = Turn(GV, Input, AirTurningSpeed, Mag); //Vector3.Lerp(GV.normalized, Input.normalized, Time.fixedDeltaTime * AirTurningSpeed).normalized * Mag;
                }
                if (GV.magnitude > MaximumSpeed)
                {
                    GV = GV.normalized * MaximumSpeed;
        ...
        if (_2DZone)
        {
            PathCreation.VertexPath _Path = _2DZone.GetActivePath();

            Vector3 _LocalPositionPath = transform.InverseTransformPoint(_Path.GetClosestPointOnPath(transform.position));
            Vector3 _LocalPosition = new Vector3(_LocalPositionPath.x, transform.InverseTransformPoint(transform.position).y, _LocalPositionPath.z);
            Vector3 _PositionPath =_Path.GetClosestPointOnPath(transform.position);

            Vector3 RelativePos = transform.position - _PositionPath;

            Vector3 Normal = _Path.GetNormalAtDistance(_Path.GetClosestDistanceAlongPath(transform.position) + 1);

            transform.position = Vector3.Lerp(transform.position, _PositionPath + Vector3.ProjectOnPlane(RelativePos, Normal), Time.fixedDeltaTime*5); 

            GV = Vector3.ProjectOnPlane(GV, Normal);
    ...
    Vector3 Turn(Vector3 V1, Vector3 V2, float TurningSpeed, float Mag)
    {
        if (Vector3.Angle(V1.normalized, V2.normalized) > 120)
        {
            return Vector3.Lerp(V1.normalized, V2.normalized, Time.fixedDeltaTime * AirTurningSpeed) * Mag;
        }
        return Vector3.Lerp(V1.normalized, V2.normalized, Time.fixedDeltaTime * AirTurningSpeed).normalized * Mag;
    }
    ...
    void HomingPhysics()
    {
        ...
        {
            Velocity = ((_HomingTarget.transform.position + _HomingTarget.GetComponent<Target>().Offset) - transform.position).normalized * Mathf.Clamp(Velocity.magnitude, MaximumSpeed / 2, MaximumSpeed);
    ...
    {
        _Ball = false;
        float RayRange = (GroundRaycastLengthMin +  Height) * 1.5f;
        ...
        if (Physics.Raycast(transform.position + -RayDirection * 0.5f, RayDirection, out _Hit, RayRange, _Ground))
        {
            ...
            _Grounded = true;
            if (Vector3.Angle(Vector3.up, _Hit.normal) > 10)
            {
                _PlayerState = PlayerState.Normal;
                return;
            }
    ...
    void WallSlidePhysics()
    {
        ...
        Velocity = -(Vector3.Cross(_GroundNormal, transform.right)) * 0.5f;

        WallTimer = WallTimer + Time.fixedDeltaTime;
        if (WallTimer > 2)
        {
            _PlayerState = PlayerState.Normal;
            NormalPhysics();
    ...
    void SideStepPhysics()
    {

        if (_PlayerState == PlayerState.RightStep)
        {
            RaycastHit _Hit;
            if (Physics.Raycast(transform.position + transform.up * 0.5f, transform.right, out _Hit, StepSpeed * Time.fixedDeltaTime, _Ground))
            {
                //Step onto wall
            ...
            {
                //SideStep
                transform.position = transform.position + transform.right * StepSpeed * Time.fixedDeltaTime;
            }
        }
        else
        {
        ...
        StepTimer = StepTimer + Time.fixedDeltaTime;
        if (StepTimer > StepTime) _PlayerState = PlayerState.Normal;
    ...
    void BoundsCheck()
    {
        ...
        if (Physics.Raycast(transform.position + transform.up * 0.5f, transform.forward, out _Hit, 0.4f, _Ground)){
            //LocalVelocity.x = 0;
            Debug.Log("Forward Colission Detected");
            WallHit(_Hit);
        }
        else if (Physics.Raycast(transform.position + transform.up * 0.5f, -transform.forward, out _Hit, 0.4f, _Ground))
        {
            //LocalVelocity.x = 0;
            Debug.Log("Forward Colission Detected");
            WallHit(_Hit);
        }
        else if (Physics.Raycast(transform.position + transform.up * 0.5f, transform.up, out _Hit, 0.4f, _Ground))
        {
            //LocalVelocity.y = 0;
            WallHit(_Hit);
        }
        else if (Physics.Raycast(transform.position + transform.up * 0.5f, Velocity.normalized, out _Hit, 0.4f, _Ground))
        {
            //LocalVelocity.y = 0;
            WallHit(_Hit);
        }
